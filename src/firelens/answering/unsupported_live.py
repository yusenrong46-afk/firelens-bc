...
_UNSUPPORTED_LIVE_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    (
        ...
        re.compile(
            ...
            rf"(?:air quality|aqhi|aqi|smoke conditions?)\b|"
            rf"\b(?:air quality|aqhi|aqi)\s+(?:in|near|around|for)\s+[A-Z]|"
            rf"\bis\s+(?:it|.{{1,50}})\s+smoky\b|"
            rf"\b{_CURRENT_CUE_TEXT}\b.{{0,60}}\b"
            rf"(?:air quality|aqhi|aqi|smoke conditions?|smoky)\b|"
            rf"\b(?:air quality|aqhi|aqi|smoke conditions?|smoky)\b.{{0,60}}"

/** Product v2 successor. Historical fullstack and Atlas v1 suites stay unchanged. */
import { openComposer, openSources, openMatchingRecords, openMapRecords, openClaimDetails, localOrigin } from "../browserNavigation";
...
const LOCAL_ORIGIN = localOrigin;
...
async function ask(page: Page, question: string): Promise<void> {
  ...
  const response = page.waitForResponse(r => r.url().endsWith("/api/v1/ask") && r.request().method() === "POST");
...
test("answers the named Mountain Fire question first and keeps official map context beside the answer", async ({
  context,
  page,
}) => {
  const attemptedExternal = await localOnly(context, page);
...
test("preserves exact structured-claim authority on the proof card", async ({
  ...
  await page.getByRole("dialog", { name: "Inspect answer evidence" }).getByText("Statement and validation", { exact: true }).click();
...
test("does not mislabel unrelated or smoke questions as reviewed evacuation guidance", async ({
  ...
  const conversation = page.getByLabel("Question and answer");
  await expect(conversation.getByText("General background · Not source-verified", { exact: true })).toBeVisible();
...
test("keeps common wildfire-mistake discussion conversational without a live lookup", async ({
  ...
  const conversation = page.getByLabel("Question and answer");
  await expect(conversation.getByText("General background · Not source-verified", { exact: true })).toBeVisible();
  await expect(conversation).toContainText("Not source-verified");
  ...
  await expect(conversation.getByText("General background · Not source-verified", { exact: true }).last()).toBeVisible();
...
test("binds an explicit map selection and asks for selection when a singular follow-up is ambiguous", async ({
  ...
  await openSources(page);
  await expect(page.getByRole("status", { name: "Answer status" }).first()).toContainText(
...
test("holds medical, jailbreak, and unit-conversion boundaries on the real stack", async ({
  ...
  await expect(conversion.getByText("General background · Not source-verified", { exact: true })).toBeVisible();
  await expect(conversion).toContainText(/Not source-verified/i);

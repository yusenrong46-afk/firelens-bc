...
export function AtlasQuestion({ session, composer, open, examplesRequested, onClose, onPrepare, onMap }: {
  ...
  return <dialog ref={dialog} className="atlas-question" aria-label="Ask FireLens" onCancel={onClose} onClose={onClose}>
    ...
    {composer}
    <button className="atlas-nearby-trigger" type="button" aria-expanded={nearbyOpen} onClick={() => { setNearbyOpen(!nearbyOpen); if (!nearbyOpen) requestAnimationFrame(() => community.current?.focus()); }}>Near me</button>
    {nearbyOpen && <section className="atlas-home__nearby" aria-label="Nearby official records">

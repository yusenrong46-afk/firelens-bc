...
| --- | ---: | ---: | --- |
| `src/firelens/agent/compose.py` | 612 | 798 | Added packet identity fields, selected-record follow-up binding, and public limitation/suggestion binding. The one-turn compose owner stays intact so live, mixed, and clarification routes keep a single result-set writer. |
...
| `src/firelens/live.py` | 739 | 734 | Corrected paginated aggregate freshness and made strict row-contract failure quarantine the affected layer. A full adapter split is deferred because the bounded safety fix does not justify moving the fail-closed fetch and normalization boundary. |

...
def _inventory_file_entry(inventory: dict[str, Any], logical_path: str) -> dict[str, Any]:
    matches = [entry for entry in inventory["files"] if entry["logical_path"] == logical_path]
    if len(matches) != 1:
        raise ValueError(
...
def _frontend_axe(axe: Any, *, context: str) -> tuple[int, dict[str, Any]]:
    ...
    for impact in expected_impact_keys:
        _strict_int(
            impact_counts,
            impact,
    ...
    *,
    ...
    for index, event in enumerate(request_events):
        ...
        if failure is not None:
            _require_nonempty_string(failure, context=f"{event_context} failure")
        if int(response_status is not None) + int(failure is not None) != 1:
...
def _frontend_map_evidence(
    ...
    *,
    ...
    return declared, declared_detail_integrity, recomputed_placement["sanity_passed"]
...
def _capture_frontend_surface(
    *,
    ...
    frontend_surface = _frontend_surface(
        report_path,
